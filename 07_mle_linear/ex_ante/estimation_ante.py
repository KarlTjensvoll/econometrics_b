...
def estimate(
    ...
    result =  None # FILL IN Use obj_func and theta0 in the optimize.minimize() function.
...
def variance(
    ...
    if cov_type == 'Hessian':
        a = result.hess_inv
        cov = None  # Fill in the first avar formula
    elif cov_type == 'Outer Product':
        cov = None  # Fill in the second avar formula
    elif cov_type == 'Sandwich':
        a = result.hess_inv
        cov = None  # Fill in the third avar formula
        
    # We need this check, so that the code can run if cov is not filled.
    if cov == None:
        pass
    else:
